...
export function getStories() {
  ...
  const storyFiles = [
    ...
    { id: 7, filename: 'a-real-peace.md', title: 'a real peace' },
  ];
  ...
  const stories = storyFiles.map(({ id, filename, title }) => {
    ...
    // Parse markdown into chapters and extract story title
    const { storyTitle, chapters } = parseMarkdownChapters(fileContents);

    return {
      id,
      title,
      storyTitle, // Title from markdown file
      chapters
    };
  });
...
function parseMarkdownChapters(markdown) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Check if this is the story title (single # at start)
